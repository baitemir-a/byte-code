...
fn firstLine(self: *const Minimap, view: *const View, line_count: usize) usize {
    ...
    const max_scroll = @as(f32, @floatFromInt(view.rowCount() - 1)) * theme.line_height;
...
pub fn handleMouse(self: *Minimap, view: *View, buf: *const core.Buffer, p: rl.Vector2, pressed: bool) bool {
    ...
    view.scrollToLine(target - view.visibleLines() / 2);
...
pub fn draw(self: *const Minimap, view: *const View, buf: *const core.Buffer, hl: *const core.syntax.Highlighter, changes: ?View.Changes) void {
    ...
    const view_first = view.topLine();
    const view_lines = view.visibleLines();
