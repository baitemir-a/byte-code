    ...
    while (self.rowAtIndex(git, n)) |row| : (n += 1) {
        ...
        switch (row) {
            ...
            .entry => |e| {
                ...
                const x = font.drawFit(entry.path[base..], r.x + GitPanel.pad + 10 + file_icon.space(), ty, text_end, theme.foreground);
...
fn drawFile(font: Font, path: []const u8, letter: u8, x0: f32, ty: f32, y: f32, right: f32, conflict: bool) void {
    ...
    const x = font.drawFit(path[base..], x0 + file_icon.space(), ty, status_x - 8, theme.foreground);
