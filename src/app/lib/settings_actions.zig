...
/// display, and needs redoing every frame anyway since raylib recomputes
/// its own on window resize.
...
pub fn openSettings(self: *App) !void {
    ...
    try self.insertTab(self.active + 1, .initSettings(self.gpa));
...
pub fn runSettingsAction(self: *App, action: SettingsPage.Action) !void {
    ...
    switch (action) {
        ...
        .toggle_format_on_save => self.settings.format_on_save = !self.settings.format_on_save,
        .toggle_trim_whitespace => self.settings.trim_trailing_whitespace = !self.settings.trim_trailing_whitespace,
        .toggle_final_newline => self.settings.insert_final_newline = !self.settings.insert_final_newline,
