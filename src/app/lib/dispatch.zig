...
pub fn execute(self: *App, cmd: core.Command) !void {
    ...
    switch (cmd) {
        ...
        .toggle_comment => try self.toggleComment(b),
        .select_next_occurrence => try self.selectNextOccurrence(b),
        .jump_to_bracket => try self.jumpToBracket(b),
        .fold => try self.foldAtCursor(),
        .unfold => self.unfoldAtCursor(),
        .fold_all => try self.foldAll(),
