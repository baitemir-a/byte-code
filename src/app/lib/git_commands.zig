...
//! undoing the last commit), the ones that finish or call off a merge,
//! and the history under the files.
...
/// Calls a half-done merge off, after asking: the conflicts already
/// sorted out are lost with it.
pub fn abortMerge(self: *App, root: []const u8) !void {
    const t = i18n.tr().git;
    if (!self.confirm(t.abort_merge_question, t.abort_merge_detail, t.abort_merge)) return;
    self.gitAction(self.git.abortMerge(self.io, root));
