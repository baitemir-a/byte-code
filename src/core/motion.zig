...
test "words and smart home" {
    ...
    try testing.expectEqual(@as(usize, 12), wordLeft(b.items(), b.cursor));
    try testing.expectEqual(@as(usize, 8), wordLeft(b.items(), 11));
    apply(&b, .line_start, false, 1);
...
test "left collapses selection" {
    ...
    try b.insert("hello");
    apply(&b, .word_left, true, 1);
    apply(&b, .char_left, false, 1);
    try testing.expectEqual(@as(usize, 0), b.cursor);
