...
test "keys, values and comments" {
    ...
    _ = try expect(Lexer.init("  \"name\": \"rl\", \"n\": -1.5e3, \"ok\": true, // note", .{}), "  \"name\": \"rl\", \"n\": -1.5e3, \"ok\": true, // note", &.{
    ...
    const lx = try expect(Lexer.init("[1, /* a", .{}), "[1, /* a", &.{ "punctuation:[", "number:1", "punctuation:,", "comment:/* a" });
