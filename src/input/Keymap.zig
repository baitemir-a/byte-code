...
pub const entries = [_]Entry{
    ...
    .{ .action = .command_palette, .group = .files, .default = primary(.p, shift), .also = &.{plain(.f1)} },
    ...
    .{
        .action = .nav_back,
        .group = .views,
        .default = if (is_mac) with(.minus, .{ .ctrl = true }) else with(.left, .{ .alt = true }),
    },
    .{
        .action = .nav_forward,
        .group = .views,
        .default = if (is_mac) with(.minus, .{ .ctrl = true, .shift = true }) else with(.right, .{ .alt = true }),
    },
    // ------------------------------------------------------------- find
    ...
    .{ .action = .select_next_occurrence, .group = .find, .default = primary(.d, .{}) },
    // ---------------------------------------------------------- editing
    ...
    .{ .action = .indent, .group = .editing, .default = plain(.tab) },
    .{
        .action = .newline,
        .group = .editing,
        .default = plain(.enter),
    ...
    .{ .action = .toggle_comment, .group = .editing, .default = primary(.slash, .{}), .also = &.{primary(.kp_divide, .{})} },
    .{ .action = .outdent, .group = .editing, .default = with(.tab, shift), .also = &.{primary(.left_bracket, .{})} },
    .{ .action = .indent_lines, .group = .editing, .default = primary(.right_bracket, .{}) },
    .{ .action = .duplicate_lines, .group = .editing, .default = primary(.d, shift) },
    .{ .action = .delete_lines, .group = .editing, .default = primary(.k, shift) },
    .{ .action = .jump_to_bracket, .group = .editing, .default = primary(.backslash, shift) },
    .{ .action = .fold, .group = .editing, .default = primary(.left_bracket, .{ .alt = true }) },
...
pub fn command(action: Action) core.Command {
    return switch (action) {
        ...
        .jump_to_bracket => .jump_to_bracket,
        .fold => .fold,
        .unfold => .unfold,
        .fold_all => .fold_all,
