...
pub fn poll(gpa: std.mem.Allocator, out: *std.ArrayList(Command)) !void {
    ...
    const left_alt_only = !is_mac and down(.left_alt) and !ctrl;
    // Option+Z / Alt+Z: word wrap. (On macOS Option+Z also types "Ω": drop it.)
    const wrap_key = pressed(.z) and !m.primary and !m.shift and (if (is_mac) alt and !ctrl else left_alt_only);
    if (wrap_key) try out.append(gpa, .toggle_word_wrap);
    while (true) {
        const cp = rl.getCharPressed();
        if (cp == 0) break;
        if (cp < 32 or cp == 127 or (ctrl and !alt) or left_alt_only or wrap_key) continue;
    ...
    // Search options, as in VS Code: Cmd+Option+C / W on macOS, Alt+C / W
    // elsewhere.
    const option_keys = if (is_mac) m.primary and alt else left_alt_only;
    if (option_keys and pressed(.c)) try out.append(gpa, .toggle_match_case);
    if (option_keys and pressed(.w)) try out.append(gpa, .toggle_whole_word);

    if (m.primary) {
        if (pressed(.a)) try out.append(gpa, .select_all);
        if (pressed(.c) and !alt) try out.append(gpa, .copy);
        ...
        if (pressed(.w) and !alt) try out.append(gpa, .close_tab);
        ...
        // Cmd+G and Cmd+Shift+G (Ctrl elsewhere): the Git view. Next /
        // previous match are Enter / Shift+Enter in the find bar, or F3.
        if (pressed(.g)) try out.append(gpa, .show_git);
    }
    ...
    // Option+Shift+Up / Down (Alt+Shift elsewhere): grow the selection to
    // the enclosing scope, or back.
    const scope_keys = m.shift and !m.primary and !ctrl and (if (is_mac) alt else down(.left_alt));
    if (scope_keys and pressed(.up)) try out.append(gpa, .expand_selection);
    if (scope_keys and pressed(.down)) try out.append(gpa, .shrink_selection);
    // Option+Up / Down (Alt elsewhere) moves the line.
    const line_keys = !m.shift and !m.primary and !ctrl and (if (is_mac) alt else down(.left_alt));
    if (line_keys and pressed(.up)) try out.append(gpa, .move_line_up);
    if (line_keys and pressed(.down)) try out.append(gpa, .move_line_down);
    ...
    for (moves) |mv| {
        if ((scope_keys or line_keys) and (mv.key == .up or mv.key == .down)) continue;
